...
parser.add_argument('-save', '--save-directory',
...
#         init_bbox = bbox_coordinates(self.x[0][0])
    ...
    def get_sample(self, idx):
        ...
        # Crop image with height and width twice the prev bounding box
        # height and width
        crop_prev = CropPrev()
    ...
    def test(self):
        ...
        for i in range(self.len):
            ...
            self.prev_rect = bb
            print('frame: %d, IoU = %f' % (
                i+2, self.axis_aligned_iou(self.gt[i], bb)))
            ax.set_axis_off()
            plt.savefig(os.path.join(self.save_dir, str(i+2)+'.jpg'))
        end = time.time()
        print('fps: {}'.format(self.len/(end-st)))


if __name__ == "__main__":
    args = parser.parse_args()
    print(args)
    tester = TesterOTB(args.data_directory,
                       args.model_weights,
                       args.save_directory)
    tester.test()
